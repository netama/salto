...
import { AdapterHTTPClient, HTTPError, HTTPReadClientInterface, HTTPWriteClientInterface } from '../client/http_client'
import { DeploymentRequestsByAction, AdapterApiConfig, getConfigWithDefault, AdapterSwaggerApiConfig } from '../config'
 ...
 */
export const oldDeploySingleChange = async <
  TCredentials,
...
>({
  ...
  fieldsToIgnore,
  queryParams,
  convertError, // TODON add default convertError implementation
  ...
  deployEqualValues?: boolean // TODON allow controlling in infra
}): Promise<ResponseResult> => {
  if (isModificationChange(change) && !deployEqualValues) {
    // TODON get rid of the flag? not sure
  ...
  try {
    ...
    if (isAdditionOrModificationChange(change)) {
      ...
      // TODON allow copying additional fields based on definitions
    }
    return response
  } catch (err) {
