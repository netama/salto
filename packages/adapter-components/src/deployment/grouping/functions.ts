...
export const getChangeGroupIdByConfig =
  <Action extends string, ClientOptions extends string>(
    groupingConfig: DefaultWithCustomizations<InstanceDeployApiDefinitions<Action, ClientOptions>>,
  ): ChangeIdFunction =>
  async change =>
    queryWithDefault(groupingConfig).query(getChangeData(change).elemID.typeName)?.changeGroupId?.(change)
