...
export const upgradeDeployTypeDefinitions = <
  Action extends string,
  ClientOptions extends string
>(oldDef: DeploymentRequestsByAction<Action>): InstanceDeployApiDefinitions<Action, ClientOptions> => ({
  requestsByAction: {
    customizations: _.mapValues(oldDef, def => collections.array.makeArray({
      request: {
        endpoint: {
          path: def?.url,
          method: def?.method,
