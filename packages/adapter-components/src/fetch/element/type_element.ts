...
import { markServiceIdField } from '../../elements/type_elements'
...
const generateNestedType = (args: lowerdashTypes.PickyRequired<GenerateTypeArgs, 'parentName'>): NestedTypeWithNestedTypes => {
  ...
  const { element: elementDef } = mergeSingleDefWithDefault(elementDefs.default, elementDefs.customizations[name]) ?? {}
  if (elementDef === undefined) {
    log.error('could not find any element definitions for type %s', name)
  }

  if (validEntries.length === 0) { // TODON handle all for top-level props in generateType
    return {
 ...
 */
export const generateType = (args: Omit<GenerateTypeArgs, 'parentName' | 'isMapWithDynamicType'>): ObjectTypeWithNestedTypes => { // TODON also create empty types for everything defined
  const {
    adapterName,
    elementDefs, // TODON rename to fetchDefs?
    typeName,
    definedTypes,
    entries,
    typeNameOverrides,
  ...
  if (elementDef === undefined) {
    ...
    // TODON make sure this is a valid scenario
    // (probably only for subtypes in cases where an elem id hasn't been defined?)
  }
  if (elementDef?.topLevel?.custom !== undefined) {
    ...
    return { type: type[0], nestedTypes } // TODON better safeties
  }
  ...
  // TODON if a type is both explicitly renamed and appears as a standalone type, warn?
  const typesToRename = computeTypesToRename({ elementDefs, typeNameOverrides })

  const naclName = naclCase(typesToRename[typeName] ?? typeName)
  const path = [
  ...
  const { elemID, hide } = topLevel ?? {}

  const type = new ObjectType({
    elemID: new ElemID(adapterName, naclName),
    fields, // TODON make sure to omit the value of omitted fields when working on the entries
    path,
    isSettings: elemID?.singleton ?? false,
    annotations: hide
      ? { [CORE_ANNOTATIONS.HIDDEN_VALUE]: true }
      : undefined,
  })

  // TODON !!! move this to type-fix since overriding the type from string to number might reverse this?
  if (resourceDef?.serviceIDFields !== undefined) {
