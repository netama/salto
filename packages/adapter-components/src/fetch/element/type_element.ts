 ...
 */
// TODON also create empty types for everything defined
export const generateType = (args: Omit<GenerateTypeArgs, 'parentName' | 'isMapWithDynamicType'>): ObjectTypeWithNestedTypes => {
  const {
    adapterName,
      ...
      .map(fieldName => [
        fieldName,
        {
          refType: addNestedType(generateNestedType({
            ...
            // TODON use better merge when ignoreDefaultFieldCustomizations is set
