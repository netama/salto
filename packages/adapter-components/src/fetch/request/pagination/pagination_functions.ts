...
import { PaginationFunc } from '../../../definitions/system/requests/pagination'
...
export const itemOffsetPagination = ({
  ...
  const nextPage: PaginationFunc = ({ currentParams, responseData }) => {
...
export const pageOffsetPagination = ({ firstPage, paginationField, pageSize, dataField }: {
  ...
  const nextPageFullPages: PaginationFunc = ({ currentParams, responseData }) => {
    const items = getItems(responseData, dataField)
...
export const offsetAndLimitPagination = ({ paginationField }: {
  ...
  const getNextPage: PaginationFunc = (
    ...
    if (!isPageResponse(responseData)) {
      // TODON decide if should pass in endpoint identifier so it can be logged - but thrown so can catch...
    ...
    const currentPageStart = _.get(responseData, paginationField) as number // TDOON avoid cast
