...
type GetEntriesParams = {
  ...
  requestsContext?: Record<string, Record<string, unknown>>
...
const getEntriesForType = async (
  ...
  const {
    ...
    requestsContext, nestedFieldFinder, computeGetArgs, adapterName,
  } = params
  ...
  const getExtraFieldValues = async (instances: InstanceElement[]):
  Promise<Record<string, Record<string, Entries>>> => {
    const nestedEntriesValues = (await Promise.all(
      (recurseInto ?? [])
        .map(async nested => {
          const relevantInstances = instances
            .filter(instance =>
              shouldRecurseIntoEntry(
                instance.value,
                (requestsContext ?? {})[instance.elemID.getFullName()],
                nested.conditions,
              ))
          ...
          const nestedRequestContext = Object.fromEntries(
            relevantInstances.map(inst => [
              ...
              Object.fromEntries(
                nested.context.map(
                  contextDef => [contextDef.name, _.get(inst.value, contextDef.fromField)]
                )
              ),
            ])
          )
          ...
          return Object.entries(nestedEntries)
            .map(([id, entries]) => ({ id, field: nested.toField, entries }))
        })
    )).flat()
    return _(nestedEntriesValues)
      .groupBy('id')
      .mapValues(val => Object.fromEntries(val.map(v => [v.field, v.entries])))
      .value()
  }
  const extraFields = await getExtraFieldValues(instanceElements)
  ...
  const instancesWithExtraValues = _.mapValues(
    idToInstances,
    instances => instances.map(instance => {
      const instanceExtraFields = extraFields[instance.elemID.getFullName()]
      Object.entries(instanceExtraFields ?? {}).forEach(([fieldName, extraEntries]) => {
        instance.value[fieldName] = extraEntries.instances.map(i => i.value)
      })
      return instance
    })
  )
  const allInstances = Object.values(instancesWithExtraValues).flat()
  const newTypeName = allInstances[0].refType.elemID.name
  const { type: newType, nestedTypes: newNestedTypes } = generateType({
    adapterName,
    name: newTypeName,
    entries: allInstances.map(inst => inst.value),
    hasDynamicFields: hasDynamicFields === true,
    transformationConfigByType,
    transformationDefaultConfig,
  })
