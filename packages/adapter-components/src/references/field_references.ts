...
const isRelativeSerializer = <CustomIndexName extends string>(
  serializer: ReferenceSerializationStrategy<CustomIndexName>,
): serializer is ReferenceSerializationStrategy<CustomIndexName> & { getReferenceId: GetReferenceIdFunc } =>
  'getReferenceId' in serializer

export const replaceReferenceValues = async <TContext extends string, CustomIndexName extends string>({
  instance,
  resolverFinder,
  elemLookupMaps,
  ...
  resolverFinder: ReferenceResolverFinder<TContext, CustomIndexName>
...
>(
  ...
  ): Promise<ReferenceSerializationStrategy<CustomIndexName> | undefined> => {
