...
const isInstanceOfCustomObjectDef = (element: Element): element is InstanceElement => (
  isInstanceElement(element) && element.type.elemID.name === CUSTOM_OBJECT_DEFINITION_TYPE
)

// TODO replace by something using annotations later, when allowing renames
const apiName = (elem: Element): string => elem.elemID.name

const createObjectFromInstance = (inst: InstanceElement): ObjectType => (
  new ObjectType({
    // TODON use annotation instead of instance name later
    elemID: new ElemID(ZUORA_BILLING, inst.elemID.name),
    fields: _.mapValues(
      {
