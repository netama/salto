...
import { DuckTypeTransformationConfig } from '@salto-io/adapter-components/src/config'
...
const filter: FilterCreator = ({ config }) => ({
  onFetch: async (elements: Element[]) => {
    ...
    const transformationByType : Record<string, DuckTypeTransformationConfig> = {}
    Object.entries(configByType).forEach(([k, v]) => ([k, v.transformation]))
